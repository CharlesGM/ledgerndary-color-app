...
5. Run Unit Tests: Executes the unit tests to ensure the PAGE_COLOUR is correctly handled.
...
- Run Unit Tests: If the validation passes, the workflow proceeds to run the unit tests. If any test fails, the workflow will also fail.

This ensures that invalid color configurations are caught early in the CI pipeline, maintaining the integrity of the application.
